%% ID 017 reslicing and preparing for manual segmentation
...
fn1 = ['C:\THG\COLE_LV_1\B_data\A_DicomInfo\DicomInfo_' id '.mat'];
pna = ['C:\THG\COLE_LV_1\B_data\E_CVI\3DCS_' id '_LGE'];
pnb = ['C:\THG\COLE_LV_1\B_data\E_CVI\3DCS_' id '_CS'];

%% load data analysis randomization
load(strcat(pwd, '\rand_data_analysis_LV_1.mat'));

% find randomization row
for j = 1:size(randomization,1)
 if strcmp(randomization{j,1},id)
    rnd = j;
 end
end; clear j
...
for k = 1:length(DicomInfo(1,lge_).Filenames)
    
    % load slice
    LGE                 = double(dicomread(DicomInfo(1,lge_).Filenames{k,1}));
    LGE_info.DicomInfo  = dicominfo(DicomInfo(1,lge_).Filenames{k,1});
    ...
    info.PatientName.FamilyName = ['3DCS_rand_' num2str(randomization{rnd,3})];
    ...
    % save CS data
    if k < 10
        dicomwrite(uint16(CS_slc),[pnb '\SAX_Series_' num2str(lge) '_0' num2str(k) '.dcm'],info,'IOD','MR Image Storage');
    elseif k <= 10
        dicomwrite(uint16(CS_slc),[pnb '\SAX_Series_' num2str(lge) '_' num2str(k) '.dcm'],info,'IOD','MR Image Storage');
    end        
...
for k = 1:length(lax_)
    ...
    % save CS data
    dicomwrite(uint16(CS_slc),[pnb '\LAX_Series_' num2str(lax(k)) '.dcm'],info,'IOD','MR Image Storage');
...
for k = 1:length(cine_)
    ...
    for l = 1:length(fn)
        ...
        info.PatientsSex = info.PatientSex;
        info.StudyDescription = '3DCS';
        info.PatientName.FamilyName = ['3DCS_rand_' num2str(randomization{rnd,2})];
